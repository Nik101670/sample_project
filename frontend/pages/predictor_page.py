...
def show_predictor_page(API_URL):
    """Display the production predictor page"""
    ...
    if len(numeric_features) > 0:
        st.header("⚙️ Manufacturing Parameters")
    ...
    if res.status_code == 200:
        ...
        with col1:
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                value=pred,
                domain={'x': [0, 1], 'y': [0, 1]},
                title={'text': "Production Output (Parts/Hour)", 'font': {'size': 20}},
                delta={'reference': 35, 'increasing': {'color': "green"}, 'decreasing': {'color': "red"}},
                gauge={
                    'axis': {'range': [0, 70], 'tickwidth': 1, 'tickcolor': "darkblue"},
        ...
        with col2:
            ...
            with st.expander("📋 Current Settings"):
                ...
                for feat in numeric_features:
                    if feat in user_data and user_data[feat] != 0:
                        st.write(f"**{feat.replace('_', ' ').title()}:** {user_data[feat]}")
